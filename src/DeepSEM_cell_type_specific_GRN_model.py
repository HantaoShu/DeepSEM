...
parser.add_argument('--alpha', type=float, default=0, help='coefficient for L-1 norm of A.')
parser.add_argument('--beta', type=float, default=0.1, help='coefficient for KL term.')
...
class celltype_GRN_model:
    ...
    def train_model(self):
        dataloader, Evaluate_Mask, num_nodes, num_genes, data, truth_edges, TFmask2, gene_name, ori_mask = self.init_data()
        adj_A_init  = self.initalize_A(data)
        vae = VAE_EAD(adj_A_init, 1, self.opt.n_hidden, self.opt.K,self.nonLinear).float().cuda()
        ...
        self.opt.save_name + '/rep_' + str(self.opt.rep) + '_epoch_' + str(epoch) + '.tsv', sep='\t', index=False)
