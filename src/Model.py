...
class InferenceNet(nn.Module):
    ...
    def reparameterize(self, mu, var,generation):
    ...
    def forward(self, x, adj, temperature=1.0,generation=False):
        ...
        z = self.reparameterize(mu, var,generation)
...
class VAE_EAD(nn.Module):
    def __init__(self, adj_A, x_dim, z_dim, y_dim,nonLinear):
    ...
    def forward(self, x,dropout_mask, temperature=1.0, opt=None,generation=False):
        x_ori = x
        ...
        out_inf = self.inference(x, adj_A_t, temperature,generation)
