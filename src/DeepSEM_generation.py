...
def init_data():
    ...
    return dataloader,  num_nodes, num_genes, data, TF_mask, gene_name, Dropout_Mask,means,stds
...
def train_model(dataloader, opt ):
    ...
    import math
    try:
        n_rep = int(math.ceil(1 / float(opt.data_file.split('_')[-1].split('.csv')[0])))
    except:
        n_rep = 1
    ...
    pkl.dump([input_xs, rec_xs, data_ids],
             open(str(opt.rep) + '_' + 'DeepSEM' + opt.data_file.split('/')[-1].split('.csv')[0] + '.pkl',
                  'wb'))


if __name__ == '__main__':
    start_time = time.time()
    dataloader,  num_nodes, num_genes, data, TFmask2, gene_name, Dropout_Mask,means,stds = init_data()
